            ...
            <script>
                $(document).ready(function(){
                    $('#transactions').DataTable({
                        ...
                        "serverSide": true,
                        "ajax":{
                            "url": "{{ url('servicetrans') }}",
                            "dataType": "json",
                            "type": "POST",
                            "data":{ _token: "{{csrf_token()}}","service":"{{ $service }}","shortcode":"{{ $shortcode }}", date_start: @json(request('start')), date_end: @json(request('end'))}
                        },
                        "columns": [
                            { "data": "shortcode" },
                            { "data": "customer_name" },
            ...
            <script>
                $(document).ready(function(){
                    $('#transactions').DataTable({
                        ...
                        "serverSide": true,
                        "ajax":{
                            "url": "{{ url('shortcodetrans') }}",
                            "dataType": "json",
                            "type": "POST",
                            "data":{ _token: "{{csrf_token()}}","shortcode":"{{ $shortcode }}", date_start: @json(request('start')), date_end: @json(request('end')) }
                        },
                        "columns": [
                            { "data": "shortcode" },
                            { "data": "customer_name" },
